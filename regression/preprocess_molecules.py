...
def torsion(xyz1, xyz2, xyz3, xyz4):
    p0, p1, p2, p3 = map(np.array, (xyz1, xyz2, xyz3, xyz4))
    b0 = -1.0 * (p1 - p0)
    b1 = p2 - p1
    b2 = p3 - p2
...
def get_atom_features(atom, mol, coords, centroid):
    ...
    features.extend([
        float_flag(atom.GetIsAromatic()), float_flag(atom.IsInRing()),
        ...
        float(atom.GetTotalNumHs()), float(atom.GetDegree()), float(atom.GetExplicitValence()),
        float(atom.GetFormalCharge()), float(atom.GetImplicitValence()), float(atom.GetNumRadicalElectrons()),
        float(props['atomic_mass']), float(props['a_num']), float(electronegativity.get(sym, 0.0)), float(vdw_radius.get(sym, 0.0)),
...
def get_bond_features(bond, coords):
    ...
    return [float(bond.GetBondTypeAsDouble()), float_flag(bond.GetIsConjugated()), float_flag(bond.GetIsAromatic()), float_flag(bond.GetStereo() == Chem.rdchem.BondStereo.STEREOZ), float_flag(bond.GetStereo() == Chem.rdchem.BondStereo.STEREOE), bond_length, bond_angle, torsion_angle]
...
def mol2graph(mol):
    ...
    x_physchem_list, x_coords_list = [], []
    for atom in mol.GetAtoms():
        features = get_atom_features(atom, mol, coords, centroid)
        x_physchem_list.append(features[:-5])
        x_coords_list.append(features[-5:])
    feats = chem_feature_factory.GetFeaturesForMol(mol)
    for f in feats:
        family = f.GetFamily()
        for atom_id in f.GetAtomIds():
            if family == 'Donor':
                x_physchem_list[atom_id][-2] = 1.0
            elif family == 'Acceptor':
                x_physchem_list[atom_id][-1] = 1.0
    edge_index_list, edge_attr_physchem_list, edge_attr_coords_list = [], [], []
    for bond in mol.GetBonds():
        u, v = bond.GetBeginAtomIdx(), bond.GetEndAtomIdx()
        features = get_bond_features(bond, coords)
        edge_index_list.append([u, v])
        edge_attr_physchem_list.append(features[:5])
        edge_attr_coords_list.append(features[5:])
    x_physchem = torch.tensor(x_physchem_list, dtype=torch.float)
    x_coords = torch.tensor(x_coords_list, dtype=torch.float)
    if edge_index_list:
        edge_index = torch.tensor(edge_index_list, dtype=torch.long).t().contiguous()
        edge_attr_physchem = torch.tensor(edge_attr_physchem_list, dtype=torch.float)
        edge_attr_coords = torch.tensor(edge_attr_coords_list, dtype=torch.float)
    else:
        edge_index = torch.LongTensor([[0], [0]])
        edge_attr_physchem = torch.FloatTensor([[0] * 5])
        edge_attr_coords = torch.FloatTensor([[0] * 3])
    return DATA.Data(
        x_physchem=x_physchem,
        x_coords=x_coords,
        edge_index=edge_index,
        edge_attr_physchem=edge_attr_physchem,
        edge_attr_coords=edge_attr_coords
    )
